package io.zell.cwg.cli;

import java.util.concurrent.Callable;
import picocli.CommandLine;
import picocli.CommandLine.Command;
import picocli.CommandLine.Mixin;

@Command(
    name = "ingest",
    mixinStandardHelpOptions = true,
    description = "Validate workload configuration before secondary-storage ingestion is implemented.")
public final class IngestCommand implements Callable<Integer> {

  @Mixin private ConfigCommandMixin config;
  @CommandLine.Spec private CommandLine.Model.CommandSpec spec;

  @Override
  public Integer call() {
    return ConfigCommands.validateFutureCommand(
        config,
        spec.commandLine(),
        "Secondary-storage ingestion is not implemented in this foundation slice.");
  }
}
